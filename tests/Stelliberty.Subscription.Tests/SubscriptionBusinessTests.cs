...
public sealed class SubscriptionBusinessTests
{
    ...
    public void PageSelectsFirstSubscriptionWhenAdded()
    {
        ...
        var page = new SubscriptionPageViewModel(subscriptionSelectionStore: selectionStore);
    ...
    public void PageExternalEditorOpensOnlyExistingSubscriptions()
    {
        ...
        var page = new SubscriptionPageViewModel(subscriptionFileOpener: opener);
    ...
    {
        var store = new FakeSubscriptionStore([Subscription("sub-1"), Subscription("sub-2")]);
        var page = new SubscriptionPageViewModel(subscriptionStore: store);
        page.LoadSubscriptions(store.LoadSubscriptions());
    ...
    public async Task ChainProxyDialogCandidateToggleRemovesSelectedNodeAndKeepsOrder()
    {
        ...
        Assert.Equal(["HK", "TW"], dialog.Slots.Select(slot => slot.NodeName));
    ...
    public async Task ChainProxyDialogAddingSameNameCustomReplacesExistingItem()
    {
        ...
        dialog.Open(
            ...
            [
                ...
                new SubscriptionCustomChainProxy("keep", "Keep", ["HK", "JP"])
            ]);
        await WaitUntilAsync(() => !dialog.IsLoading);
        ...
        var dialog = new SubscriptionChainProxyDialogViewModel(contextLoader: subscriptionId =>
        {
            if (subscriptionId == "sub-1")
            {
                ...
                try
                {
                    ...
                    return new SubscriptionChainProxyContext(["Old chain"], [new ChainProxyNodeOption("Old", "ss")]);
                }
                finally
                {
            ...
            return new SubscriptionChainProxyContext(["New chain"], [new ChainProxyNodeOption("New", "trojan")]);
        });
